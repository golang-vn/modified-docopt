...
# Modified docopt 0.6.2. (c)opyleft by tuanhace 2023-12-20. All rights removed.
#
# Installation:
#   Step 1. Install docopt
#       $ pip3 install docopt==0.6.2
#   Step 2. Download modified docopt.py
#       $ curl -LO https://raw.githubusercontent.com/golang-vn/modified-docopt/master/docopt.py
#   Step 3. Replace original docopt.py
...
#
# Usage:
#   Step 1. Modify your code to retrieve one more variable (e.g: remain_str) from docopt return
...
#   Step 2. Do whatever you want with this variable
...
def docopt(doc, argv=None, help=True, version=None, options_first=False):
    ...
    * Full documentation is available in README.rst as well as online
    ...
    if matched:
        collected_dic = Dict((a.name, a.value) for a in (pattern.flat() + collected))
        left_str = ""
        for each_item in left:
            if isinstance(each_item, Option):
                k = each_item.short if each_item.short is not None else each_item.long if each_item.long is not None else ""
                v = wrapDoublequote(each_item.argcount * str(each_item.value))
                if k:
                    # Method 1: Simple just a space, quick used for both short and long options
                    #left_str = f"{left_str} {k} {v}"
                    # Method 2: More beautiful
                    if v and k.startswith("--"):
                        left_str = f"{left_str} {k}={v}"
                    else:
                        left_str = f"{left_str} {k}{v}"
                    #print("[DEBUG] Option(k={}, v={}) left_str == {}".format(k, v, left_str))
            elif isinstance(each_item, Argument):
                v = wrapDoublequote(str(each_item.value)) if each_item.value is not None else ""
                if v:
                    left_str = f"{left_str} {v}"
                    #print("[DEBUG] Argument(v={}) left_str == {}".format(v, left_str))
        return collected_dic, left_str.strip()
